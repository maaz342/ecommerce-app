...
export default async function Page({searchParams}:Props) {
  ...
  const sizeFiltler=size?`&& "${size}"in sizes`:""
  const searchFilter=search?`&& name match "${search}"`:""
//search for search prooduct
  const Filter=`*[${productFilter}${colorFilter}${categoryFilter}${sizeFiltler}${searchFilter}]`
    ...
    <div>
      ...
      <ProductSort />
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className={cn("grid grid-cols-1 gap-x-8 gap-y-10", products && products.length > 0 ? 'lg:grid-cols-4' : 'lg:grid-cols-[1fr_3fr]')}>
